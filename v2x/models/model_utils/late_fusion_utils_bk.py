...
class BBoxList(object):
    def __init__(self, boxes, dir, label, score, dist, class_score=None):
        """
        Data format:
...
class BasicFuser(object):
    ...
    def bayesian_fusion(self, match_score_vec):
        ...
        fused_positive = np.exp(np.sum(log_positive_scores, axis=0))
        fused_negative = np.exp(np.sum(log_negative_scores, axis=0))
